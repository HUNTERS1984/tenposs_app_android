 ...
 * {@code nextBoolean()}, {@code nextDouble()}, {@code nextInt()},
 * {@code nextLong()}, {@code nextString()} or {@code nextNull()}. Writers
 * should make exactly one call to one of <code>value()</code> or
 * <code>nullValue()</code>. For arrays, type adapters should start with a call
 * to {@code beginArray()}, convert all elements, and finish with a call to
 * {@code endArray()}. For objects, they should start with {@code beginObject()}
 * , convert the object, and finish with {@code endObject()}. Failing to convert
 * a value or converting too many values may cause the application to crash.
 * 
 * <p>
...
public abstract class TypeAdapter<T> {
	 ...
	 * call {@link #write(JsonWriter, Object)} for
	 * lenient writing.
	 * 
	 * @param value
	 ...
	 * similar {@link Gson#fromJson(Reader, Class) fromJson} method,
	 ...
	 * {@code JsonReader} and call {@link #read(JsonReader)} for lenient
	 * reading.
	 * 
	 * @return the converted Java object. May be null.
	 * @since 2.2
