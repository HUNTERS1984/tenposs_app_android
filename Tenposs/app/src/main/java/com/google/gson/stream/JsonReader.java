...
public class JsonReader implements Closeable {
	...
	/**
	 * Returns the {@link JsonToken#STRING string} value
	 ...
	 *             closed.
	 */
	public int nextString(OutputStreamWriter out) throws IOException {
		int p = peeked;
		if (p == PEEKED_NONE) {
			p = doPeek();
		}
		...
		if (p == PEEKED_UNQUOTED) {
			// result = nextUnquotedValue(out);
			out.write(nextUnquotedValue());
		} else if (p == PEEKED_SINGLE_QUOTED) {
			result = nextQuotedValue('\'', out);
		} else if (p == PEEKED_DOUBLE_QUOTED) {
			result = nextQuotedValue('"', out);
		} else if (p == PEEKED_BUFFERED) {
			// result = peekedString;
			out.write(peekedString);
			peekedString = null;
		} else if (p == PEEKED_LONG) {
			// result = Long.toString(peekedLong);
			out.write(Long.toString(peekedLong));
		} else if (p == PEEKED_NUMBER) {
			// result = new String(buffer, pos, peekedNumberLength);
			out.write(buffer, pos, peekedNumberLength);
	...
	/**
	 * Returns the {@link JsonToken#BOOLEAN boolean}
	...
	/**
	 * Returns the {@link JsonToken#NUMBER double} value
	...
	/**
	 * Returns the {@link JsonToken#NUMBER long} value of
	 ...
	 */
	private int nextQuotedValue(char quote, OutputStreamWriter writer) throws IOException {
		// Like nextNonWhitespace, this uses locals 'p' and 'l' to save
		// inner-loop field access.
		char[] buffer = this.buffer;
	...
	/**
	 * Returns the {@link JsonToken#NUMBER int} value of
	 * the next token, consuming it. If the next token is a string, this method
