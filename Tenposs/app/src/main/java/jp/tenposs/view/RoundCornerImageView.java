...
import static jp.tenposs.view.RoundCornerDrawable.BOTTOM_LEFT;
import static jp.tenposs.view.RoundCornerDrawable.BOTTOM_RIGHT;
import static jp.tenposs.view.RoundCornerDrawable.TOP_LEFT;
import static jp.tenposs.view.RoundCornerDrawable.TOP_RIGHT;
...
public class RoundCornerImageView extends ImageView {
    ...
    private ColorStateList mBorderColor =
            ColorStateList.valueOf(RoundCornerDrawable.DEFAULT_BORDER_COLOR);
    ...
    public RoundCornerImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        float cornerRadiusOverride =
                a.getDimensionPixelSize(R.styleable.RoundCornerImageView_riv_corner_radius, -1);

        mCornerRadii[TOP_LEFT] =
                a.getDimensionPixelSize(R.styleable.RoundCornerImageView_riv_corner_radius_top_left, -1);
        mCornerRadii[TOP_RIGHT] =
                a.getDimensionPixelSize(R.styleable.RoundCornerImageView_riv_corner_radius_top_right, -1);
        mCornerRadii[BOTTOM_RIGHT] =
                a.getDimensionPixelSize(R.styleable.RoundCornerImageView_riv_corner_radius_bottom_right, -1);
        mCornerRadii[BOTTOM_LEFT] =
                a.getDimensionPixelSize(R.styleable.RoundCornerImageView_riv_corner_radius_bottom_left, -1);
        ...
        mBorderWidth = a.getDimensionPixelSize(R.styleable.RoundCornerImageView_riv_border_width, -1);
        if (mBorderWidth < 0) {
            mBorderWidth = DEFAULT_BORDER_WIDTH;
        }

        mBorderColor = a.getColorStateList(R.styleable.RoundCornerImageView_riv_border_color);
        if (mBorderColor == null) {
            mBorderColor = ColorStateList.valueOf(RoundCornerDrawable.DEFAULT_BORDER_COLOR);
        }

        mMutateBackground = a.getBoolean(R.styleable.RoundCornerImageView_riv_mutate_background, false);
        mIsOval = a.getBoolean(R.styleable.RoundCornerImageView_riv_oval, false);

        final int tileMode = a.getInt(R.styleable.RoundCornerImageView_riv_tile_mode, TILE_MODE_UNDEFINED);
        ...
        final int tileModeX =
                a.getInt(R.styleable.RoundCornerImageView_riv_tile_mode_x, TILE_MODE_UNDEFINED);
        if (tileModeX != TILE_MODE_UNDEFINED) {
            setTileModeX(parseTileMode(tileModeX));
        }

        final int tileModeY =
                a.getInt(R.styleable.RoundCornerImageView_riv_tile_mode_y, TILE_MODE_UNDEFINED);
    ...
    public void setImageBitmap(Bitmap bm) {
        mResource = 0;
        mDrawable = RoundCornerDrawable.fromBitmap(bm);
        updateDrawableAttrs();
        super.setImageDrawable(mDrawable);
    }
    ...
    private Drawable resolveResource() {
        ...
        return RoundCornerDrawable.fromDrawable(d);
    }
    ...
    private void updateBackgroundDrawableAttrs(boolean convert) {
        if (mMutateBackground) {
            if (convert) {
                mBackgroundDrawable = RoundCornerDrawable.fromDrawable(mBackgroundDrawable);
            }
    ...
    private void updateAttrs(Drawable drawable, ScaleType scaleType) {
        ...
        if (drawable instanceof RoundCornerDrawable) {
            ...
            if (mCornerRadii != null) {
                ((RoundCornerDrawable) drawable).setCornerRadius(
                        mCornerRadii[TOP_LEFT],
                        mCornerRadii[TOP_RIGHT],
                        mCornerRadii[BOTTOM_RIGHT],
                        mCornerRadii[BOTTOM_LEFT]);
    ...
    public void setCornerRadius(float topLeft, float topRight, float bottomLeft, float bottomRight) {
        if (mCornerRadii[TOP_LEFT] == topLeft
                && mCornerRadii[TOP_RIGHT] == topRight
                && mCornerRadii[BOTTOM_RIGHT] == bottomRight
                && mCornerRadii[BOTTOM_LEFT] == bottomLeft) {
            return;
        }

        mCornerRadii[TOP_LEFT] = topLeft;
        mCornerRadii[TOP_RIGHT] = topRight;
        mCornerRadii[BOTTOM_LEFT] = bottomLeft;
        mCornerRadii[BOTTOM_RIGHT] = bottomRight;
    ...
    public void setBorderColor(ColorStateList colors) {
        ...
        mBorderColor =
                (colors != null) ? colors : ColorStateList.valueOf(RoundCornerDrawable.DEFAULT_BORDER_COLOR);
        updateDrawableAttrs();
        updateBackgroundDrawableAttrs(false);
        if (mBorderWidth > 0) {
            invalidate();
